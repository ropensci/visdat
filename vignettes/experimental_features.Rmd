...
`vis_dat_ly(airquality)` is a faster version of calling `ggplotly(vis_dat(airquality))`. When hovering the cursor over a cell, it will display information about each each cell, the type of data, the value, the row number, and the variable.

## vis_miss_ly

`vis_miss_ly(airquality)` is similar to `vis_dat_ly` - a faster version of calling `ggplotly(vis_miss(airquality))`. When hovering the cursor over a cell, it will display information about each each cell, the type of data, the number of missings in that case, and the number of missings in that variable.
...
Where the `expectation` would get captured and passed to `mutate_if`, which would need to have a way of dispatching different `is.` arguments.
